...
Ext.define('DecisionLink.controller.Navigation', {
    ...
    config: {
        refs: {
            ...
            bizModelsList: 'opportunitycarousel #competitorsList',
        ...
        control: {
            ...
            "opportunitycarousel #opportunitySituationPanel #saveSituationButton": {
    ...
    onAccountScenariosListItemTap: function(dataview, index, target, record, e, eOpts) {
        var carousel,
            ...
            cost_light_image = 'http://test.decisionlink.com/' + record.get('cost_light_image'),
            benefit_light_image = 'http://test.decisionlink.com/' + record.get('benefit_light_image'),
            me = this;
        ...
        DecisionLink.app.setCurrentTerm(term);
    ...
    onPasswordfieldAction: function(textfield, e, eOpts) {
        var username,
            password,
            usernameField,
            passwordField,
    ...
    onOpptyCarouselActivate: function(newActiveItem, container, oldActiveItem, eOpts) {
        ...
        // Load in scenario business_models
        me.getScenarioBizModels(user_id, id, function(store) {
            me.getBizModelsList().setStore(store);
        });
    ...
    onSQQListItemTap: function(dataview, index, target, record, e, eOpts) {
        ...
        var term = DecisionLink.app.getCurrentTerm();
        for(i = 2; i <= 5; i++) {
            curr = details.child('#year' + i + 'container').child('#year' + i + 'growth');
            curr_val = record.get('sqq_y' + i) * 100;
            curr.setValue( curr_val );
            curr_label = details.child('#year' + i + 'container').child('#year' + i + 'label');
            curr_label.setHtml('<p>' + curr_val + '</p>');
            if(i <= term) {
                curr.setHidden(false);
                curr_label.setHidden(false);
            } else {
                curr.setHidden(true);
                curr_label.setHidden(true);
            }
        }
    ...
    onSaveSQQButtonTap: function(button, e, eOpts) {
        ...
        sqqPanel = Ext.ComponentQuery.query('salesquestiondetailpanel')[0];
        ...
        metric_id = sqqPanel.child('#metricIdField').getValue();
        ...
        sqq_y5 = ( sqqPanel.child('#year5container').child('#year5growth').getValue() ) / 100;
    ...
    onSaveVFButtonTap: function(button, e, eOpts) {
        ...
        sqq_y1 = ( VFPanel.child('#year1container').child('#year1growth').getValue() ) / 100;
    ...
    onSaveSituationButtonTap: function(button, e, eOpts) {
        var SituationPanel,
            ...
            id = DecisionLink.app.getCurrentOpptyId(),
            scenario,
            bizModelsList = Ext.ComponentQuery.query('opportunitycarousel #opportunitySituationPanel #bizModelsList')[0].getSelection(),
            bizModels = '',
            competitorsList = Ext.ComponentQuery.query('opportunitycarousel #opportunitySituationPanel #competitorsList')[0].getSelection(),
            competitors = '',
            areas,
            industriesList = Ext.ComponentQuery.query('opportunitycarousel #opportunitySituationPanel #industriesList')[0].getSelection(),
            industries = '',
            baselines,
            wacc,
            term,
        ...
        for(i = 0; i < bizModelsList.length; i++) {
            bizModels += bizModelsList[i].data.id + ',';
        }
        bizModels = bizModels.substring(0, bizModels.length - 1);

        for(i = 0; i < competitorsList.length; i++) {
            competitors += competitorsList[i].data.id + ',';
        }
        competitors = competitors.substring(0, competitors.length - 1);

        for(i = 0; i < industriesList.length; i++) {
            industries += industriesList[i].data.id + ',';
        }
    ...
    getScenarioBizModels: function(user_id, oppty_id, callback) {
        var store = Ext.data.StoreManager.lookup('ScenarioBizModelsStore'),
            url = DecisionLink.app.getServiceUrl() + '/GetSellingScenario1.php' +
            '?user_id=' + user_id +
            '&oppty_id=' + oppty_id;
        store.getProxy().setUrl(url);
    ...
    saveSellingScenario: function(user_id, id, scenario, competitors, areas, industries, baselines, wacc, term, business_models, callback) {
        var store = Ext.data.StoreManager.lookup('SuccessStore'),
            url = DecisionLink.app.getServiceUrl() + '/SaveSellingScenario1.php' +
            '?user_id=' + user_id +
            '&id=' + id +
            '&scenario=' + scenario +
    ...
    saveOpportunityCost: function(oppty_id, cost_type_id, accrual_type_id, cost, callback) {
        var store = Ext.data.StoreManager.lookup('SuccessStore'),
            cost = DecisionLink.app.removeCommas(cost),
    ...
    saveValueFormulaMetrics: function(metric_id, sqq_value, sqq2_value, impact, oppty_id, unit_type_id, impact_source_type_id, sqq_y1, sqq_y2, sqq_y3, sqq_y4, sqq_y5, apply_all, callback) {
        var store = Ext.data.StoreManager.lookup('SuccessStore'),
            sqq_value = DecisionLink.app.removeCommas(sqq_value),
            sqq2_value = DecisionLink.app.removeCommas(sqq2_value),
            impact = DecisionLink.app.removeCommas(impact),
    ...
    updateOpportunitySQQ: function(sqq_id, sqq_override, source_type_id, oppty_id, metric_id, sqq_y2, sqq_y3, sqq_y4, sqq_y5, callback) {
        var store = Ext.data.StoreManager.lookup('SuccessStore'),
            sqq_override = DecisionLink.app.removeCommas(sqq_override),
    ...
    saveOpportunity: function(company_id, user_id, name, wacc, oppty_revenue, solutions, callback) {
        var store = Ext.data.StoreManager.lookup('SuccessStore'),
            oppty_revenue = DecisionLink.app.removeCommas(oppty_revenue),
